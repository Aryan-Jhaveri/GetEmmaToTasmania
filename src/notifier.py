...
<style>
  ...
  .insights {{ background: white; border-radius: 12px; padding: 20px;
               margin-bottom: 16px; border-left: 4px solid #0071e3; }}
  .insights h3 {{ font-size: 15px; font-weight: 700; margin: 0 0 12px; color: #1d1d1f; }}
  .insight-row {{ font-size: 14px; color: #1d1d1f; padding: 5px 0;
  ...
  .clanker {{ font-size: 11px; color: #aeaeb2; margin-top: 12px; font-style: italic; }}
...
<div class="container">
  ...
  <div class="footer">
    ...
    <a class="sheet-link" href="{sheet_link}">View full price history &amp; charts in Google Sheets</a><br>
  ...
  {secondary_buttons}
    ...
    def _insights_block(
        ...
        # How many flights / dates were scanned
        total = len(raw_offers)
        dep_dates = sorted({o.departure_date for o in raw_offers})
        rows.append(
            f'<div class="insight-row">'
            f'<span class="insight-label">Flights scanned today:</span> '
            f'<b>{total} flights</b> across <b>{len(dep_dates)} departure dates</b> '
            f'({dep_dates[0]} – {dep_dates[-1]})'
            f'</div>'
        )

        # Cheapest departure date overall (across all routes)
        cheapest_offer = min(raw_offers, key=lambda o: o.price_cad)
        rows.append(
            f'<div class="insight-row">'
            f'<span class="insight-label">Cheapest departure date seen:</span> '
            f'<b>{cheapest_offer.departure_date}</b> — '
            f'{cheapest_offer.route_key} for <b>{cheapest_offer.price_display}</b> '
            f'({cheapest_offer.airlines}, {cheapest_offer.num_stops} stop{"s" if cheapest_offer.num_stops != 1 else ""})'
            f'</div>'
        )

        # Price range across everything checked
        prices = [o.price_cad for o in raw_offers]
        rows.append(
            f'<div class="insight-row">'
            f'<span class="insight-label">Price range checked:</span> '
            f'<b>${min(prices):,.0f}</b> – <b>${max(prices):,.0f} CAD</b> '
            f'(your budget: ${threshold:,.0f})'
            f'</div>'
        )

        # Most common airline(s) among the cheapest offer per departure date
        cheapest_per_date = {}
        for o in raw_offers:
            key = (o.route_key, o.departure_date)
            if key not in cheapest_per_date or o.price_cad < cheapest_per_date[key].price_cad:
                cheapest_per_date[key] = o
        airline_counts: Counter = Counter(
            o.airlines for o in cheapest_per_date.values() if o.airlines
        )
        if airline_counts:
            ...
            rows.append(
                ...
                f'<b>{top_airline}</b> '
                f'(cheapest option on {top_count} of {len(cheapest_per_date)} date/route combos)'
                f'</div>'
            )

        # Historical comparison per deal route
        for deal in deals:
            prev = historical_mins.get(deal.route_key)
            if prev:
                diff = deal.price_cad - prev
                if diff < 0:
                    verdict = f'<b style="color:#30d158">New all-time low by ${abs(diff):,.0f}!</b>'
                elif diff == 0:
                    verdict = '<b>Matches the all-time low</b>'
                else:
                    verdict = f'${diff:,.0f} above the all-time low of ${prev:,.0f}'
                rows.append(
                    f'<div class="insight-row">'
                    f'<span class="insight-label">{deal.route_key} vs. history:</span> '
                    f'{verdict}'
                    f'</div>'
                )
            else:
                rows.append(
                    f'<div class="insight-row">'
                    f'<span class="insight-label">{deal.route_key} vs. history:</span> '
                    f'First price ever recorded — no baseline yet'
                    f'</div>'
                )
